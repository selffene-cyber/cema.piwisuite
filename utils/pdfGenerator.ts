...
export async function generateEvaluationPDF(result: any, formData: any, chartRef: any) {
  ...
  const variables = [
    ['Ancho de Banda', `${formData.beltWidth}"`, result.breakdown.beltWidth],
    ['Velocidad', `${formData.beltSpeed} fpm`, result.breakdown.beltSpeed],
    ['Empalme', formData.spliceType, result.breakdown.spliceType],
    ['Abrasividad', formData.abrasiveness, result.breakdown.abrasiveness],
    ['Humedad', formData.moisture, result.breakdown.moisture],
  ];
